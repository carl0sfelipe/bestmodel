...
- Verification: `uv run pytest apps/public-api/tests/test_catalog_routes.py -q` passed; `uv run pytest tests/test_session_contract.py -q` passed. The specified `/usr/bin/python3 -m pytest` command remains unavailable because system Python has no pytest module.
...
- Verification: `npm --prefix apps/web-next run build` passed. The feed fallback remains usable when either catalog request fails; it is not treated as a fatal catalog load.
