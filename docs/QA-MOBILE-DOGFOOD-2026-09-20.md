...
| M3 | **No device detection on prod.** web-next has no client-side hardware hint at all. The legacy tour's WebGPU probe maps vendor strings for nvidia/amd/intel/apple only; a Qualcomm Adreno adapter falls through (code read) to the same toast the live session showed on the no-adapter path: "No WebGPU detection here — pick a reference rig", followed by the "Pick your machine" sheet whose first 8 items are CMP 170HX, RTX 3090, RTX 3060 ×2, RTX 3060, Arc Pro B70, RTX 5090, Ryzen AI Max 395, Radeon AI Pro R9700 ×3 — no phone. | `apps/web/site/assets/mobile-page.mjs` `guessRigByVendor`; `qa_s25_legacy_tour_autodetect.png` | UX gap |
| M4 | **The agent surface advertises a mobile tour that 404s on prod.** `www.bestmodel.run/llms.txt` lists `m/index.html?as=human — mobile` and `hardware.html?as=human`; on web-next `/m/[slug]` is the *model* page, so `/m/index.html` → "404 \| This page could not be found." (live) and `/hardware.html` → 404. The tour only exists on the Pages mirror, which direction v3 D5 declares "preview, never prod". | `apps/web-next/public/llms.txt` lines 8–10, 60–61; `qa_s25_mobile_tour_404.png` | D5 line (copy bug) |
...
| M12 | **Layout at 412 px is fine.** web-next is mobile-first (360 px base, min-width queries, tables scroll in their own container); the viewport meta is present. Live: `document.documentElement.scrollWidth` = `window.innerWidth` = 412 on `/` and `/wall`; tap targets and text legible; no clipping on `/`, `/hardware`, `/submit`. | `apps/web-next/app/globals.css`; live console; `qa_s25_hardware_phone_cards.png` | pass |
...
| M14 | **The CLI cannot answer either, and its transfer path is dead against the repo pool.** `canirunit suggest --gpu snapdragon-8-elite-12gb --task decode_tok_s --runs pool.json --gpus gpu_transfer_specs.json` → `match_class: unknown`, exit 3; the "closest rig ids" hint lists `arc-b580-12gb`, `rtx-3060-12gb` (string similarity on `12gb`). `gpu_transfer_specs.json` holds 5 desktop GPUs with `gpu-*` ids that never equal pool rig keys (`rtx-3090-24gb`), so `transfer_suggestions` never fires for *any* GPU with the in-repo snapshot (verified: `--gpu gpu-rtx-3080` → unknown too); and the factor is an fp16-TFLOPS ratio (compute-bound, written for diffusion) — the wrong estimator for bandwidth-bound LLM decode. Incident record: `docs/incidents/2026-09-20-cli-nao-responde-hardware-fora-do-catalogo-ia-fez-workaround.md`. | `cli/canirunit/src/transfer.rs`, `cli/canirunit/gpu_transfer_specs.json`; live run of `target/debug/canirunit` | capability gap (S32g) |
