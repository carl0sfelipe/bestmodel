...
with the S25 Ultra Chrome user agent (`SM-S938B`, Android 16) and browsed
in a 412×915 CSS-px viewport; the legacy GitHub Pages mirror checked where
prod points to it; repo sources read at `main` `16cf014`. Each finding
names the file that produces it.

| # | Finding | Evidence | Severity |
|---|---|---|---|
| M1 | **The phone cannot be picked on the home.** The "02 the machine" select offers the top **24** rigs by run count; the only phone-class rigs in the pool have 1 and 4 runs and never make the cut. A phone user gets a desktop GPU pre-selected and the claims-tier fallback of *other* rigs. | `apps/web-next/app/page.tsx` `RIG_LIMIT = 24`; `hardware.json` run counts | blocker for the mobile journey |
...
| M6 | **`/hardware` buries phones.** Cards are ordered by run count only; the TECNO POVA is card 101 of 185 and the Snapdragon 888 card 140 of 185 (ties by run count make the exact slot vary), both with "bandwidth unstated". | `apps/web-next/app/hardware/page.tsx`; sort of `hardware.json` | UX gap |
...
| M12 | **Layout at 412 px is fine.** web-next is mobile-first (360 px base, min-width queries, tables scroll in their own container); the viewport meta is present. No horizontal overflow observed on `/`, `/hardware`, `/wall`, `/submit`. | `apps/web-next/app/globals.css`; live check | pass |
